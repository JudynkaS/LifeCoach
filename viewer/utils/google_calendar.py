...
def create_coach_calendar_event(coach_profile, summary, description, start_dt, end_dt, timezone_str='UTC'):
    ...
    event = {
        ...
        'conferenceData': {
            'createRequest': {
                'requestId': f"{coach_profile.id}-{start_dt.timestamp()}",
    ...
    # Přidej odkaz na Google Meet do popisu
    if 'conferenceData' in created_event and 'entryPoints' in created_event['conferenceData']:
        ...
        created_event['description'] = f"{description}\n\nInformace pro připojení ke Google Meet\nOdkaz na videohovor: {meet_link}"
        
        # Aktualizuj událost s novým popisem
...
def update_coach_calendar_event(coach_profile, event_id, summary, description, start_dt, end_dt, timezone_str='UTC'):
    """
    Aktualizuje existující událost v Google kalendáři kouče.
    coach_profile: Profile instance kouče (musí mít google_refresh_token)
    event_id: ID existující události
    summary: Název události
    description: Popis události
    start_dt, end_dt: datetime (aware)
    timezone_str: např. 'Europe/Prague'
