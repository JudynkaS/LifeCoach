...
class SessionForm(BaseStyledForm):
    class Meta:
        model = Session
        fields = ['service', 'date_time', 'type', 'notes', 'meeting_url', 'meeting_address']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            # Pro klienta: vše readonly pokud je méně než 24h do začátku
                ...
                if self.instance.date_time and (self.instance.date_time - timezone.now()).total_seconds() < 24*60*60:
                    for field in self.fields:
                        self.fields[field].disabled = True
                        self.fields[field].widget.attrs['readonly'] = True
                else:
                    # Pokud je více než 24h, může měnit pouze date_time a notes
                    readonly_fields = ['service', 'type', 'meeting_url', 'meeting_address']
                    for field in readonly_fields:
                        if field in self.fields:
                            self.fields[field].disabled = True
                            self.fields[field].widget.attrs['readonly'] = True
...
class BookingForm(BaseStyledForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'meeting_url': forms.URLInput(attrs={'placeholder': 'https://...', 'class': 'form-control'}),
            'meeting_address': forms.TextInput(attrs={'placeholder': 'Address for personal session', 'class': 'form-control'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and hasattr(self.user, 'profile'):
            if self.user.profile.is_coach:
                self.fields['service'].queryset = Service.objects.filter(coach=self.user, is_active=True)
            else:
        ...
        if self.instance and self.instance.pk and self.instance.date_time:
            ...
            if not any(val == dt_str for val, _ in choices):
                display = dt.strftime('%A %d.%m.%Y %H:%M')
                choices.insert(0, (dt_str, f"Current: {display}"))
        ...
        if self.instance and self.instance.pk:
            ...
            if self.user and hasattr(self.user, 'profile') and self.user.profile.is_client:
    ...
    def clean_date_time(self):
        ...
        try:
            ...
            raise forms.ValidationError('Invalid date/time format. Please select from the available options.')
